...
void NewLMTree::try_merge(Node *node, Node *father, Dataset &dataset) {
    ...
    while (true) {
        ...
        for (std::int64_t i = 0; i < children.size(); ++i) {
            for (std::int64_t j = 0; j < children.size(); ++j) {
                ...
                if (i == j) {
                    ...
                    //     auto predicted_position_in_ei = seg.forward(min_dist_from_ej_to_ei);
                    //     assert(std::isfinite(predicted_position_in_ei));
                    //
                    //     auto real_position = std::min<std::int64_t>(seg.items.size() - 1,
                    //                                                 std::max<std::int64_t>(
                    //                                                     0, predicted_position_in_ei));
                    //     while (real_position > 0 && ei->items[real_position].first >
                    //            min_dist_from_ej_to_ei) {
                    //         --real_position;
                    //     }
                    //     while (real_position < seg.items.size() && ei->items[real_position].first <
                    //            min_dist_from_ej_to_ei) {
                    //         ++real_position;
                    //     }
                    //     if (std::abs(predicted_position_in_ei - real_position) > epsilon) {
                    //         delta_costs[i][j] = -1; //can not merge
                    //         continue;
                    //     }
                    // }
                    if (1) {
                    ...
                    //calculate the distances from data in ej to ei's center
        ...
        assert(max_i != max_j);
        //center is the same, do not need change first
    ...
    //update linear model
    // if (0) {
    //     //update inner node model(do not need)
    //     auto new_node = new Node();
    //     new_node.center = node->center;
    //     new_node.radius = node->radius;
    //     for (auto &seg: node->segments) {
    //         for (auto &item: seg.items) {
    //             new_node.add_sub_node(distance(dataset[node->center],
    //                                            dataset[item.second.child->center]), item.second.child);
    //         }
    //     }
    //     add_pivot_model_for_inner(new_node, dataset);
...
void NewLMTree::check_adding_pivot() const {
    ...
    while (!routes.empty()) {
        ...
        // std::cout << node->segments.size() << std::endl;
        if (!node->is_leaf) {
            for (auto &item: node->items) {
                routes.push(item.second.child);
            }
...
bool NewLMTree::insert_with_pivot(const std::int64_t insert_id, Dataset &dataset, Node *node, Segment *father_seg,
    ...
    if (node->is_leaf) {
        ...
        assert(nearest_seg_id != -1);
        auto &seg = node->segments[nearest_seg_id];
        // !!! the distance of segment is the new point to center
        seg.min_dist_to_center = std::min(seg.min_dist_to_center, dist_to_center);
        seg.max_dist_to_center = std::max(seg.max_dist_to_center, dist_to_center);
        ...
        //update the min-max distance to center in each segment
    ...
    if (node->cost > 2 * node->last_split_cost) {
        if (node->segments.size() > 2 * node->old_children_size) {
    ...
    while (!nodePQ.empty()) {
        ...
        if (element.node->is_leaf) {
            if (element.is_seg) {
                ...
                for (auto &item: seg.items) {
                    //minimum predict distance to a data
    ...
    while (!routes.empty()) {
        ...
        if (node->is_leaf) {
            ...
            for (const auto &seg: node->segments) {
                ...
                const auto min_dist = dist_query_pivot - radius; // if x < this continue
                const auto max_dist = dist_query_pivot + radius; // if x > this break;
            ...
            for (const auto &seg: node->segments) {
                ...
                auto min_dist = dist_query_pivot - radius; // if x < this continue
                auto max_dist = dist_query_pivot + radius; // if x > this continue
                auto begin_idx = static_cast<std::int64_t>(0);
                // auto end_idx = std::min<std::int64_t>(static_cast<std::int64_t>(seg.items.size())-1,
                //                         std::max<std::int64_t>(0,seg.forward(max_dist)));
                //
                // while (end_idx < seg.items.size() && seg.items[end_idx].first - seg.items[end_idx].
                //        second.child->radius <= max_dist) {
                //     ++end_idx;
                // }
                // end_idx = seg.items.size();
                auto end_idx = seg.items.size();
                for (std::int64_t i = begin_idx; i < end_idx; i++) {
                    const auto dist_to_pivot = seg.items[i].first;
                    const auto child = seg.items[i].second.child;
