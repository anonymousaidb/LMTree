...
void Dataset::writeVectorToDirectory(const Dataset &result, const std::string &path) {
    ...
    const std::int64_t batch_size_bytes = 1024 * 1024 * 32; // 32MB per batch
...
std::tuple<float, float, float> linearFit(const std::vector<std::pair<float, float> > &data) {
    if (data.size() < 2) {
        throw std::invalid_argument("需要至少两个数据点来拟合直线");
    }
    ...
    float numerator = 0.0f; // (x_i - mean_x)(y_i - mean_y)
    float denominator = 0.0f; // (x_i - mean_x)^2
    ...
    if (denominator == 0.0f) {
        throw std::runtime_error("x 值都相同，无法拟合直线");
    }

    float slope = numerator / denominator;
    float intercept = mean_y - slope * mean_x;

    // 计算最大误差（非标准 OLS 目标，但可用于评估拟合效果）
