...
namespace ML {
    ...
    Matrix<T, Allocator>::Matrix(const Matrix &matrix) noexcept: rows(matrix.rows),
                                                                    cols(matrix.cols) { // NOLINT(*-explicit-constructor)
        _data_ptr = allocator.allocate(rows * cols);
        ++pointer_count;
    ...
    Matrix<T, Allocator>::Matrix(Matrix &&matrix) noexcept : rows(matrix.rows), cols(matrix.cols),
                                                                           _data_ptr(matrix._data_ptr) { // NOLINT(*-explicit-constructor)
    ...
    Matrix<T, Allocator> Matrix<T, Allocator>::broadcast(const std::int64_t new_rows, const std::int64_t new_cols) const {
        ...
        for (std::int64_t i = 0; i < rows; i++) {//copy columns
    ...
    Matrix<T, Allocator> Matrix<T, Allocator>::inverse() const {
        ...
        for (std::int64_t _0 = 0; _0 < cols; _0++) {
            ...
            // get_pointer_position the max one (main element)
            if (max_index != _0) {
                //exchange tow rows
    ...
    void test() {
        ...
        // torch::Tensor b = torch::tensor(std::vector(a.data_ptr(),a.data_ptr()+a.size())).view({nn,dd});
        ...
        // std::cout <<b.mean(1).sizes()<< std::endl;
        // std::cout <<b.mean(1)<< std::endl;
        // auto mean = a.mean(ML::Matrix<float>::col);
        // std::cout <<mean<< std::endl;


        // std::cout <<b.mean(0).sizes()<< std::endl;
        // std::cout <<b.mean(0)<< std::endl;
        // auto mean = a.mean(ML::Matrix<float>::row);
        // std::cout <<mean<< std::endl;

        // std::cout <<b.var(1).sizes()<< std::endl;
        // std::cout <<b.var(1)<< std::endl;
