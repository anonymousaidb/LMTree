        ...
        if (find_queryLower >= find_queryUpper) {
            if (find_queryLower == find_queryUpper && find_queryLower >= query_lower &&
                ...
                for (int x = pos_lower; x <= pos_upper; ++x) {
                    if (CaculateEuclideanDis(mainRef_point.iValuePts[x], queryPoint, disType) <= radius) {
...
double RangeQuery::CaculateEuclideanDis(Point &point_a, Point &point_b, int distance_type) {
    disOpts++;
    double total = 0.0;
