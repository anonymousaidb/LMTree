...
public:
    ...
    KNN(Point &, mainRef_Point &, priority_queue<pair<double, unsigned int>> &, CalCirclePos &,
        vector<CalCirclePos> &, unsigned, int8_t (*)[1000000], unsigned);
    ...
    void DoRangeQueryForKNN(unsigned, vector<vector<int>> &, unsigned long long, mainRef_Point &,
                            priority_queue<pair<double, unsigned int>> &, int8_t (*)[1000000]);
};

KNN::KNN() {}

KNN::KNN(Point &KNN_pt, mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int>> &KNNRes_queue,
         CalCirclePos &mainRefPtCircle, vector<CalCirclePos> &ref_query, unsigned k, int8_t (*arr)[1000000],
         unsigned clu_id) {
...
void KNN::DoRangeQueryForKNN(unsigned i, vector<vector<int>> &rank_list, unsigned long long queryRange,
                             mainRef_Point &mainRef_point, priority_queue<pair<double, unsigned int>> &KNNRes_queue,
                             int8_t (*arr)[1000000]) {
    if (i == rank_list.size() - 1) {
        ...
        while (low <= high) {
            ...
            {
                if ((middle > 0 && mainRef_point.iValuePts[middle - 1].i_value < query_lower) || (middle == 0))
                {
                    break;
                } else
                    high = middle - 1;
        ...
        if (find_queryLower >= find_queryUpper) {
            if (find_queryLower == find_queryUpper && find_queryLower >= query_lower &&
                ...
                for (int x = pos_lower; x <= pos_upper; ++x) {
                    if (arr[clu_id][x] == 0) {
                        arr[clu_id][x] = 1;
                        double dis_pt_qryPt = CaculateEuclideanDis(mainRef_point.iValuePts[x], queryPoint, disType);
...
int KNN::BinarySearch(vector<double> &disList, double target, int low, int high) {
    ...
    while (low <= high) {
        ...
        if (target < disList[middle]) {
            ...
            if ((middle > 0 && disList[middle - 1] < target) || (middle == 0))
            {
                break;
            } else
                high = middle - 1;
...
double KNN::CaculateEuclideanDis(Point &point_a, Point &point_b, int distance_type) {
    disOpts++;
    double total = 0.0;
