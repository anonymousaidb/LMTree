...
CaculateIValue::CaculateIValue(Clu_Point &clu, mainRef_Point mainRefPoint) {
    ...
    for (int i = 0; i < num_data; i++) {
        ...
        double dis_pt_mainRefPt = CaculateEuclideanDis(cluster.clu_point[i], mainRef_point.point, disType);
...
double CaculateIValue::CaculateEuclideanDis(Point &point_a, Point &point_b, int distanceType) {
    disOpts++;
    double total = 0.0;
    switch (distanceType) {
        case 2:
