...
namespace mvp {
    ...
    public:
        ...
        const std::vector<mvp::item_t<T>> KNNQuery(const T &target, int k, long &count) const;
...
const std::vector<mvp::item_t<T>>
mvp::MVPTree<T, BF, PL, LC, LPN, FO, NS>::KNNQuery(const T &target, int k, long &count) const {
    if (k <= 0 || m_top == NULL) {
        return std::vector<mvp::item_t<T>>();
    }

    double RK = rak;


    auto cmp = [](const std::pair<double, item_t<T>> &a,
                  const std::pair<double, item_t<T>> &b) {
        return a.first < b.first;
    };

    std::priority_queue<
            std::pair<double, item_t<T>>,
            std::vector<std::pair<double, item_t<T>>>,
            decltype(cmp)
    > heap(cmp);
    ...
    while (iter++ < max_iter) {
        std::vector<item_t<T>> candidates = Query(target, RK);
        bool updated = false;

        for (const auto &item: candidates) {
            double d = target.distance(item.key);
            count++;
            if (heap.size() < k) {
                heap.push({d, item});
                updated = true;
        ...
        if (heap.size() >= k) {
            double current_radius = heap.top().first;
            if (RK >= current_radius) {
                break;
            }
            if (first_k) {
                first_k = false;
...
template<typename T, int BF, int PL, int LC, int LPN, int FO, int NS>
const std::vector<mvp::item_t<T>>
mvp::MVPTree<T, BF, PL, LC, LPN, FO, NS>::Query(const T &target, const double radius) const {
