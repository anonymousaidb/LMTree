...
struct M_Node_St {
    ...
    vector<pair<double, double>> hyper_rings;
...
public:
    ...
    [[nodiscard]] int Get_Pivot_Filter_Num() const { return Pivot_Filter_Num; }

    [[nodiscard]] int Get_Cal_Distance_Num() const { return Cal_Distance_Num; }

    [[nodiscard]] int Get_Via_subNode_Num() const { return Via_subNode_Num; }

    [[nodiscard]] int Get_Via_Node_Num() const { return Via_Node_Num; }
...
private:
    ...
    void subRange_Search(M_Node_St **cur_node_address_, vector<double> &q_feature_val_, double search_range_,
                         vector<pair<double, int>> &res_vec_, double &dist_parent_q_, vector<double> &dist_q_pivot_);
    ...
    bool Is_Full(M_Node_St *node_) const { return node_->ptr_sub_tree.size() >= M; }

    size_t CalculateNodeMemoryUsage(const M_Node_St *node) const;
...
public:
    void k_NN_Search(vector<double> &q_feature_val_, int k, vector<pair<double, int>> &res_vec_);

private:
    struct QueueEntry {
        M_Node_St *node;
        double d_min;
        QueueEntry(M_Node_St *n, double d) : node(n), d_min(d) {}
    };

    struct CompareQueueEntry {
        bool operator()(const QueueEntry &a, const QueueEntry &b) {
            return a.d_min > b.d_min;
        }
    };

    void k_NN_NodeSearch(M_Node_St *node, vector<double> &q_feature_val_, int k,
