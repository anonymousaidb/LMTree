...
void PM_Tree::k_NN_Search(vector<double> &q_feature_val_, int k, vector<pair<double, int>> &res_vec_) {
    Via_Node_Num = 0;
    Via_subNode_Num = 0;
    Pivot_Filter_Num = 0;
    Cal_Distance_Num = 0;
    ...
    priority_queue<QueueEntry, vector<QueueEntry>, CompareQueueEntry> PR;
    vector<pair<double, int>> NN;
    double dk = numeric_limits<double>::max();

    PR.push(QueueEntry(root, 0.0));
    Via_Node_Num++;

    while (!PR.empty()) {
        QueueEntry entry = PR.top();
        PR.pop();
        M_Node_St *node = entry.node;

        if (entry.d_min > dk) continue;

        k_NN_NodeSearch(node, q_feature_val_, k, PR, NN, dk, dist_q_pivot);
    }
...
void PM_Tree::k_NN_NodeSearch(M_Node_St *node, vector<double> &q_feature_val_, int k,
                              priority_queue<QueueEntry, vector<QueueEntry>, CompareQueueEntry> &PR,
                              vector<pair<double, int>> &NN, double &dk, vector<double> &dist_q_pivot) {
    Via_Node_Num++;

    if (Is_leaf_Node(node)) {
        for (M_Node_St *data_entry: node->ptr_sub_tree) {
            Via_subNode_Num++;

            if (!Is_Pivot_Filter(dist_q_pivot, dk, data_entry)) {
                Pivot_Filter_Num++;
                continue;
            }
            ...
            if (dist < dk || NN.size() < static_cast<size_t>(k)) {
                NN.push_back(make_pair(dist, data_entry->id));
        ...
        for (M_Node_St *routing_entry: node->ptr_sub_tree) {
            Via_subNode_Num++;

            if (!Is_Pivot_Filter(dist_q_pivot, dk, routing_entry)) {
                Pivot_Filter_Num++;
                continue;
            }
            ...
            double d_min = max(dist - routing_entry->range, 0.0);
            double d_max = dist + routing_entry->range;

            if (d_min <= dk) {
                if (d_max < dk) {
                    dk = d_max;
                    while (!NN.empty() && NN.back().first > dk) {
                        NN.pop_back();
                    }
                    if (NN.size() < static_cast<size_t>(k)) {
                        dk = numeric_limits<double>::max();
                    } else {
                        dk = NN.back().first;
                    }
                }

                PR.push(QueueEntry(routing_entry, d_min));
            }
        }
    }
}
...
size_t PM_Tree::GetIndexMemoryUsage() const {
    return CalculateNodeMemoryUsage(root);
}

size_t PM_Tree::CalculateNodeMemoryUsage(const M_Node_St *node) const {
    if (node == nullptr) {
        return 0;
    }

    size_t memoryUsage = sizeof(*node);
    memoryUsage += node->feature_val.capacity() * sizeof(double);
    memoryUsage += node->pivot_distance.capacity() * sizeof(double);
    memoryUsage += node->ptr_sub_tree.capacity() * sizeof(M_Node_St *);
    memoryUsage += node->hyper_rings.capacity() * sizeof(std::pair<double, double>);

    for (const auto &subNode: node->ptr_sub_tree) {
        memoryUsage += CalculateNodeMemoryUsage(subNode);
    }
...
void PM_Tree::Insert(vector<double> feature_val_, int id_) {
    ...
    if (root == nullptr) {
        M_Node_St *new_root = new M_Node_St(nullptr, 1, -1, -1);
...
void PM_Tree::subRange_Search(M_Node_St **cur_node_address_, vector<double> &q_feature_val_, double search_range_,
                              vector<pair<double, int>> &res_vec_, double &dist_parent_q_,
                              vector<double> &dist_q_pivot_) {
    double dist = 0;
    M_Node_St *cur_node_ = *cur_node_address_;
    this->Via_Node_Num++;
    if (Is_leaf_Node(cur_node_)) {
        for (int i = 0; i < cur_node_->ptr_sub_tree.size(); ++i) {
            if (fabs(dist_parent_q_ - cur_node_->ptr_sub_tree[i]->dist_to_parent) <= search_range_ &&
                Is_Pivot_Filter(dist_q_pivot_, search_range_, cur_node_->ptr_sub_tree[i])) {
                this->Via_subNode_Num++;
        ...
        for (int i = 0; i < cur_node_->ptr_sub_tree.size(); ++i) {
            if (fabs(dist_parent_q_ - cur_node_->ptr_sub_tree[i]->dist_to_parent) <=
                search_range_ + cur_node_->ptr_sub_tree[i]->range) {
                this->Via_subNode_Num++;
                dist = MyFunc::Cal_Euclidean_distance(cur_node_->ptr_sub_tree[i]->feature_val, q_feature_val_);
                if (dist <= search_range_ + cur_node_->ptr_sub_tree[i]->range &&
                    Is_Pivot_Filter(dist_q_pivot_, search_range_, cur_node_->ptr_sub_tree[i])) {
                    subRange_Search(&(cur_node_->ptr_sub_tree[i]), q_feature_val_, search_range_, res_vec_, dist,
                                    dist_q_pivot_);
                }
            }
        }
...
void PM_Tree::Sub_Traverse_Get_Volumem(M_Node_St *cur_node_, double &volume_) {
    if (Is_leaf_Node(cur_node_))
        return;
    for (int i = 0; i < cur_node_->ptr_sub_tree.size(); ++i) {
        double v = MyFunc::Cal_Hypersphere_Volume(cur_node_->ptr_sub_tree[i]->range,
                                                  cur_node_->ptr_sub_tree[i]->feature_val.size());
        volume_ += v;
...
inline bool PM_Tree::Is_Pivot_Filter(vector<double> &dist_q_pivot_, double range_, M_Node_St *cur_node_) {
    if (Is_Data_Node(cur_node_)) {
        ...
        for (int i = 0; i < HR; ++i) {
            if ((dist_q_pivot_[i] - range_ > cur_node_->hyper_rings[i].second) ||
                (dist_q_pivot_[i] + range_) < cur_node_->hyper_rings[i].first) {
                Pivot_Filter_Num++;
                return false;
            }
        }
...
M_Node_St **PM_Tree::Get_Next_Node_And_Update_Range(M_Node_St **cur_node_address_, M_Node_St **insert_node_address_) {
    ...
    for (int i = 0; i < cur_node_->ptr_sub_tree.size(); ++i) {
        double dist = MyFunc::Cal_Euclidean_distance(cur_node_->ptr_sub_tree[i]->feature_val,
                                                     insert_node_->feature_val);
